...
export function AdminDashboard() {
  ...
  const cards = [
    { label: 'Модераторы', count: moderators?.length ?? 0, to: '/admin/moderators', color: 'bg-blue-50 text-blue-700' },
    { label: 'Ученики', count: students?.length ?? 0, to: '/admin/students', color: 'bg-green-50 text-green-700' },
    { label: 'Классы', count: classes?.length ?? 0, to: '/admin/classes', color: 'bg-purple-50 text-purple-700' },
    { label: 'Тесты', count: tests?.length ?? 0, to: '/admin/tests', color: 'bg-orange-50 text-orange-700' },
  ]
